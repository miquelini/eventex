...
class EmailBackendTest(TestCase):
    def setUp(self):
        ...
        UserModel.objects.create_user(
                                    username='antonio',
                                    email='antonio@carlos.net',
                                    password='abracadabra')
        self.backend = EmailBackend()
        
    def test_authenticate_with_email(self):
        user = self.backend.authenticate(email='antonio@carlos.net',
                                         password='abracadabra')
...
class MultipleEmailsTest(TestCase):
    def setUp(self):
        ...
        UserModel.objects.create_user(username='user1',
            email='antonio@garcia.net', password='abracadabra')
        UserModel.objects.create_user(username='user2',
            email='antonio@garcia.net', password='abracadabra')
        self.backend = EmailBackend()
        
    def test_multiple_emails(self):
        user = self.backend.authenticate(email='antonio@garcia.net',
                                         password='abracadabra')
        self.assertIsNone(user)
...
class FunctionalEmailBackendTest(TestCase):
    ...
    def test_login_with_email(self):
        result = self.client.login(email = 'antonio@carlos.net',
                                    password = 'abracadabra')
        self.assertTrue(result)
        
    def test_login_with_username(self):
        result = self.client.login(username = 'antonio@carlos.net',
                                   password = 'abracadabra')
        self.assertTrue(result)